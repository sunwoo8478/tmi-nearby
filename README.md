...
| Safety actions | 게시물 숨기기, 작성자 차단, 게시물/댓글/작성자 반복 신고 방지, 작성 쿨다운, 민감어/전화번호/위치 특정 표현 필터 |
...
| Safety basics | `escapeHtml()` 기반 XSS 방지, 작성 쿨다운, 민감어/전화번호/위치 특정 표현 필터, 숨김/게시물 신고/댓글 신고/작성자 신고/차단 처리 |
...
| Test / CI | `node --check`, `node:test`, Playwright E2E, GitHub Actions |
| Backend candidate | Node 내장 `http` + `node:sqlite` (`server/`, 새 의존성 없음, 프론트와는 아직 미연동) |
...
`docs/BACKEND.md`에 문서로만 있던 API 설계를 실제로 구현한 독립 서버입니다. **프론트엔드(`src/app.js`)는 아직 이 서버를 호출하지 않고** 지금처럼 `localStorage` 기반으로 그대로 동작합니다 — 연동은 별도 작업입니다.

- 새 npm 의존성 없이 Node 내장 `http` + `node:sqlite`(`DatabaseSync`)만 사용
- SQLite 파일로 영속 저장 (서버 재시작해도 데이터 유지)
- 익명 세션, 반경 기반 피드, 게시물/댓글/반응/투표, 신고·숨김·차단 API 14개 전부 구현

```bash
npm run server        # http://localhost:8787
npm run test:server   # 31개 테스트
```
